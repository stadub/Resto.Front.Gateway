...
namespace Resto.Front.Api.SamplePlugin.NotificationHandlers
{
    public class BeforeOrderBillHandler : IDisposable
    {
        ...
        private void OnBeforeOrderBill(IOrder order, [CanBeNull] IViewManager viewManager)
        {
            ...
            var message = $"Allow bill operation of the order: #{order.Number}, guests {order.Guests.Count}, sum: {order.ResultSum}?";
            var allowOperation = viewManager?.ShowYesNoPopup("Sample", message) // prefer built-in popups
