...
namespace Resto.Front.Api.SamplePlugin
{
    internal sealed class EditorTester : IDisposable
    {
        ...
        private void AddProduct()
        {
            ...
            var addedItem = PluginContext.Operations.AddOrderProductItem(42m, product, order, guest, null, credentials);

            if (product.ImmediateCookingStart)
                PluginContext.Operations.PrintOrderItems(credentials, PluginContext.Operations.GetOrderById(order.Id), new[] { addedItem });
        }
        ...
        private void DeletePrintedProduct()
        {
            ...
            {
                PluginContext.Operations.DeletePrintedOrderItems(comment, WriteoffOptions.WithoutWriteoff(removalType), order, itemsToDelete, credentials);
                return;
            }
            if (removalType.WriteoffType.HasFlag(WriteoffType.Cafe))
            {
                PluginContext.Operations.DeletePrintedOrderItems(comment, WriteoffOptions.WriteoffToCafe(removalType), order, itemsToDelete, credentials);
                return;
            }
            if (removalType.WriteoffType.HasFlag(WriteoffType.Waiter))
            {
                PluginContext.Operations.DeletePrintedOrderItems(comment, WriteoffOptions.WriteoffToWaiter(removalType), order, itemsToDelete, credentials);
                return;
            }
            if (removalType.WriteoffType.HasFlag(WriteoffType.User))
            {
                var user = PluginContext.Operations.GetUsers().First(u => u.IsSessionOpen);
                PluginContext.Operations.DeletePrintedOrderItems(comment, WriteoffOptions.WriteoffToUser(removalType, user), order, itemsToDelete, credentials);
                return;
            }
        ...
        private IDeliveryOrder CreateDelivery(bool withOriginName)
        {
            ...
            var entities = SubmitChanges(editSession);
        ...
        private static void MarkOrderAsTab()
        {
            if (!PluginContext.Operations.GetHostTerminalsGroup().IsTabMode)
            {
            ...
            var credentials = PluginContext.Operations.GetCredentials();
            var order = PluginContext.Operations.GetOrders().Last(x => x.Status == OrderStatus.New);
            var customer = PluginContext.Operations.TryGetClientById(order.CustomerIds.FirstOrDefault());

            var tabName = customer?.Name ?? $"Kirill {order.Number}";
            PluginContext.Operations.MarkOrderAsTab(tabName, order, credentials);
            order = PluginContext.Operations.GetOrderById(order.Id);
        ...
        {
            var lastOrder = PluginContext.Operations.GetOrders().LastOrDefault();
            var lastOrderItemsCount = lastOrder?.Items.Count ?? 0;
